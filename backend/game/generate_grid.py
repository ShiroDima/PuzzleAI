...
import copy
from backend.schema import Puzzle
...
class PuzzleGrid:
    def __init__(
        ...
        self.grid: Puzzle = self._create_grid(grid_size=self.grid_size)
        ...
        self.word_pos = []
        self.answers: Dict[str, List[Tuple[int, int]]] = {}

    def _create_grid(self, grid_size: Tuple[int, int]) -> Puzzle:
        return Puzzle(
            grid=[["#" for _ in range(grid_size[1])] for _ in range(grid_size[0])]
        )

    def place_words(self, search_words: List[str]) -> None:
        print(search_words)
        ...
        self.words: List[str] = [
            word for word in search_words if len(word) <= self.max_word_length
        ]
        ...
        for word in self.words:
            ...
            grid = self.grid.model_copy(deep=True)
    ...
    def _get_start_pos(self, rem_pos: Set) -> Optional[Tuple[int, int]]:
